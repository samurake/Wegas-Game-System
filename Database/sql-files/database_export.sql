...
-- Exporting database
FOR position IN
  (SELECT * FROM ALL_OBJECTS WHERE OWNER = 'WEGASGAME') LOOP
    BEGIN

      UTL_FILE.PUT_LINE(expData,dbms_metadata.get_ddl('TABLE',position.OBJECT_NAME,'WEGASGAME'));
  ...
  FOR position IN
  (SELECT * FROM ALL_OBJECTS WHERE OWNER = '') LOOP
    BEGIN
     IF position.OBJECT_TYPE != 'LOB' AND  position.OBJECT_TYPE != 'TABLE'  AND  position.OBJECT_TYPE != 'INDEX' THEN
        IF position.OBJECT_NAME != 'RECREATE_TABLES' THEN
            UTL_FILE.PUT_LINE(expData,dbms_metadata.get_ddl(position.OBJECT_TYPE,position.OBJECT_NAME,'WEGASGAME'));
        END IF;    
    END IF;  
        EXCEPTION WHEN OTHERS THEN NULL;
    END;
